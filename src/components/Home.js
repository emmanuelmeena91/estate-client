...
function Home() {
  ...
  return (
<div className="home p-4 bg-blue-500 text-white">
  <h1 className="text-3xl font-semibold mb-8 text-center">House Listings</h1>
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
    {houses.map((house) => (
      <Link key={house.id} to={`/house/${house.id}`} className="house-card bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 ease-in-out transform hover:scale-105">
        <div className="relative">
          <img src={house.image_paths} alt={house.title} className="w-full h-36 object-cover rounded-t-xl" />
